...
function eval_wind(sim, model_data, sens_type = 'ground') {
    const settings = wind_settings
    // import model data
    shared.initModel(sim, model_data);
    // add data to model
    // ***************** TO CHANGE WIND STATION DATA *****************
    const weather_stn = 'S24'
    // ***************** TO CHANGE WIND STATION DATA *****************
...
//         console.log('simulating', type, 'for file:',file)
...
//             const col_range = [0, 100];
//             outDir = '_wind/'
//             const result = eval_wind(sim, gen)
...
//         fs.writeFileSync(output_dir + outDir + file, solar_model)
//     }
// }
// run(process.argv[2])
// =================================================================================================
...
async function run(type) {
    ...
    for (const file of files) {
        ...
        const p = new Promise((resolve, reject) => {
            try {
                exec(`node genScript/sim_execute_single ${type} ${file}`, 
                (error, stdout, stderr) => {
                    if (error) {
        ...
        queues.push(p)
        if (queues.length >= PROCESS_LIMIT) {
