...
export function raster_to_sim_sky(bounds, data, info, gridSize) {
    const projObj = _createProjection(data.proj)
    ...
    console.log('data.extent', data.extent, width, height)

    const sim = new SIMFuncs()

    const extent_coords = [
        [0,                              0,                           0],
        [gridSize * data.data[0].length, 0,                           0],
        [gridSize * data.data[0].length, gridSize * data.data.length, 0],
        [0,                              gridSize * data.data.length, 0]
    ]
    const bound_coords = []
    for (const latlong of bounds) {
        const coord = [ ...projWGS84.forward(latlong), 0]
    ...
    console.log('extent_coords', extent_coords)
    const extent_ps = sim.make.Position(extent_coords)
    const extent_pgon = sim.make.Polygon(extent_ps)
    const normal = sim.calc.Normal(extent_pgon, 1)
    if (normal[2] < 0) {
        sim.edit.Reverse(extent_pgon)
    }
    ...
    for (let i = 0; i < data.data.length; i++) {
        ...
        for (let j = 0; j < data.data[i].length; j++) {
            ...
            col.push(check)
            if (check) {
                if (info.id === 'wind' && data.data[i][j] < 0.0001) { continue }
                colRange[0] = Math.min(data.data[i][j], colRange[0])
                colRange[1] = Math.max(data.data[i][j], colRange[1])    
            }
        }
        pointCheck.push(col)
    }
    ...
    if (info.col_range_rev) {
        const temp = colRange[0]
        colRange[0] = colRange[1]
        colRange[1] = temp
    }
    if (info.id === 'sky') {
        const temp = 7.13 - (6.51 * colRange[0])
        colRange[0] = 7.13 - (6.51 * colRange[1])
        colRange[1] = temp
        // colRange[0], colRange[1] = colRange[1], colRange[0]
    }

    const colorScale = chromaScale(info.col_scale).domain(colRange);
    for (let i = 0; i < data.data.length; i++) {
        for (let j = 0; j < data.data[i].length; j++) {
            if (pointCheck[i][j]) {
                let v = data.data[i][j]
                if (info.id === 'sky') { v = 7.13 - (6.51 * v) }
                values.push(v)
                context.fillStyle = colorScale(v).css();
    ...
    return [sim, projWGS84.inverse(extent2.bottom_left), colRange, canvas]
}
